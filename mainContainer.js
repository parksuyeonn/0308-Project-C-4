...
pokemonBoxStyle("200px", "200px", "white", "1px solid black");
let array = [];
let newArrLength = 0;
do {
  for (let i = 0; i < 6; i++) {
...
const firstImgBox = randomContainer.children[0];

mainContainer.children[0].addEventListener('click', function () {
  let a = Math.floor(Math.random() * 1000 + 1);
  const _URL = `https://pokeapi.co/api/v2/pokemon/${a}`;
  const xhr = new XMLHttpRequest();
